...
pro plotDspacVsChi, base, globalbase, sets, peaks, correctcenter, dynamic, savemovie
...
for i=0, nSets-1 do begin
	...
	fits[i] = experiment->latticeStrain(sets[i])
...
plotTestLatticeStrains, base, data, fits, nSets, nUsePeak, peakIndex, usePeak, legend, experiment->getWavelength(), correctcenter, dynamic, savemovie, filename
end
...
if (WIDGET_INFO(stash.correctcenter, /BUTTON_SET) eq 1) then correctcenter = 1 else correctcenter = 0
if (WIDGET_INFO(stash.savemovie, /BUTTON_SET) eq 1) then savemovie = 1 else savemovie = 0
CASE ev.id OF
	stash.input:
	else: begin
		CASE uval OF
		'PLOT': plotDspacVsChi, stash.input, stash.base, sets, peaks, correctcenter, 0, savemovie
		'DYNAMICPLOT': plotDspacVsChi, stash.input, stash.base, sets, peaks, correctcenter, 1, savemovie
...
correctcenter = WIDGET_BUTTON(buttons, VALUE='Correct center in plot', UVALUE='NOTHING')
savemovie = WIDGET_BUTTON(buttons, VALUE='Save dynamic plot in mpeg', UVALUE='NOTHING')
Widget_Control, correctcenter, Set_Button=0
; peaklist
...
stash = {base: base, input: input, plotwhatPeak:plotwhatPeak, listSets:listSets, correctcenter: correctcenter, savemovie: savemovie}
